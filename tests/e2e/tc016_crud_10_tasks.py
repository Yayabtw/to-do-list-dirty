...
class TC016TestRunner:
    ...
    def create_task(self, task_title):
        ...
        try:
            ...
            submit_button = self.driver.find_element(By.CSS_SELECTOR, "input[type='submit'].btn-info")
            submit_button.click()
    ...
    def run_test(self):
        ...
        try:
            ...
            if count_after_creation == expected_count:
                print(f"✅ Comptage correct: {count_after_creation} (attendu: {expected_count})")
            else:
                print(f"❌ Comptage incorrect: {count_after_creation} (attendu: {expected_count})")
                result['status'] = 'failed'
                result['errors'].append(
                    f"Comptage après création: {count_after_creation} != {expected_count}"
                )
            ...
            if final_count == self.initial_task_count:
                print(f"✅ Comptage final correct: {final_count} (attendu: {self.initial_task_count})")
            else:
                print(f"❌ Comptage final incorrect: {final_count} (attendu: {self.initial_task_count})")
                result['status'] = 'failed'
                result['errors'].append(
